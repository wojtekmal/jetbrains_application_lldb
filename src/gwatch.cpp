#include <iostream>
#include <string>
#include <sys/types.h>
#include <sys/ptrace.h>
#include <sys/wait.h>
#include <sys/user.h>
#include <cstdint>
#include <fcntl.h>
#include <elf.h>
#include <cstring>
#include <map>

struct SymbolInfo
{
    uintptr_t address;
    uint64_t size;
};

//struct DwarfInfo
//{
//    uintptr_t address;
//    uint64_t size;
//    bool is_signed;
//    bool is_dwarf_present;
//};

SymbolInfo get_symbol_info(char* symbol, char* path)
{
    int buffer_size = 1024 * 1024;
    char* buffer = new char[buffer_size];
    int fd = open(path, O_RDONLY);
    read(fd, buffer, buffer_size);

    auto elf_header = reinterpret_cast<Elf64_Ehdr*>(buffer);
    auto section_headers = reinterpret_cast<Elf64_Shdr*>(buffer + elf_header->e_shoff);
    Elf64_Shdr symbol_table_header;
    Elf64_Sym* symbol_table;
    char* symbol_string_table;

    for (int i = 0; i < elf_header->e_shnum; i++)
    {
        if (section_headers[i].sh_type == SHT_SYMTAB)
        {
            symbol_table_header = section_headers[i];
        }
    }

    symbol_table = reinterpret_cast<Elf64_Sym*>(buffer + symbol_table_header.sh_offset);
    symbol_string_table = buffer + section_headers[symbol_table_header.sh_link].sh_offset;
    int symbol_count = symbol_table_header.sh_size / sizeof(Elf64_Sym);
    Elf64_Sym symbol_entry;

    for (int i = 0; i < symbol_count; i++)
    {
        if (strcmp(symbol, symbol_string_table + symbol_table[i].st_name) == 0)
        {
            symbol_entry = symbol_table[i];
        }
    }

    delete[] buffer;
    return {static_cast<uintptr_t>(symbol_entry.st_value), symbol_entry.st_size};
}

//DwarfInfo get_dwarf_info(char* symbol, char* path)
//{
//    int buffer_size = 1024 * 1024;
//    char* buffer = new char[buffer_size];
//    int fd = open(path, O_RDONLY);
//    read(fd, buffer, buffer_size);
//
//    auto elf_header = reinterpret_cast<Elf64_Ehdr*>(buffer);
//    auto section_headers = reinterpret_cast<Elf64_Shdr*>(buffer + elf_header->e_shoff);
//    char* section_name_string_table = elf_header-
//}

uint64_t get_current_value(SymbolInfo symbol_info, pid_t pid)
{
    uint64_t answer = ptrace(PTRACE_PEEKDATA, pid, symbol_info.address, nullptr);

    if (symbol_info.size < 8)
    {
        answer &= (1 << (8 * symbol_info.size)) - 1;
    }

    return answer;
}

bool stopped_because_of_read(pid_t pid)
{
    uint64_t dr6 = ptrace(PTRACE_PEEKUSER, pid, offsetof(user, u_debugreg[6]), nullptr);
    if (dr6 & 1) return true;
    else return false;
}

int main(int argc, char** argv)
{
    char* symbol = argv[2];
    char* path = argv[4];

    pid_t pid = fork();

    if (pid == 0)
    {
        ptrace(PTRACE_TRACEME, 0, nullptr, nullptr);
        char* args[2] = {path, nullptr};
        execve(path, args, nullptr);
    }
    else
    {
        auto symbol_info = get_symbol_info(symbol, path);
        ptrace(PTRACE_POKEUSER, pid, offsetof(user, u_debugreg[0]), symbol_info.address);
        ptrace(PTRACE_POKEUSER, pid, offsetof(user, u_debugreg[1]), symbol_info.address);
        uint64_t current_value = get_current_value(symbol_info, pid);

        uint64_t dr7 = ptrace(PTRACE_PEEKUSER, pid, offsetof(user, u_debugreg[7]), nullptr);
        dr7 &= ~(0b11111111ULL << 16);
        std::map<uint64_t, uint64_t> size_masks = {{1, 0}, {2, 1}, {4, 3}, {8, 2}};
        dr7 |= (size_masks[symbol_info.size] << 18) + (3ULL << 16) + 3;
        dr7 |= (size_masks[symbol_info.size] << 22) + (1ULL << 20) + 12;
        bool first_stop = true;

        while (true)
        {
            ptrace(PTRACE_POKEUSER, pid, offsetof(user, u_debugreg[7]), dr7);
            ptrace(PTRACE_CONT, pid, nullptr, nullptr);
            int status;
            waitpid(pid, &status, WUNTRACED);

            if (WIFSTOPPED(status) && !first_stop)
            {
                if (stopped_because_of_read(pid))
                {
                    std::cout << symbol << "    read     " << current_value << "\n";
                }
                else
                {
                    uint64_t new_value = get_current_value(symbol_info, pid);
                    std::cout << symbol << "    write    " << current_value << " -> " << new_value << "\n";
                    current_value = new_value;
                }
            }
            else if (first_stop)
            {
                first_stop = false;
            }
            else
            {
                break;
            }
        }
    }
}